...
class Conversion:
    ...
    def _convert_currency_via_floatrates(
        ...
        if to_lower not in data:
            raise ValueError(
                f"Moneda destino '{to_code}' no encontrada en la respuesta de floatrates para '{from_code}'"
            )
    ...
    def parse_input_and_convert(self, input: str):
        ...
        if "and" in parts:
            ...
            if len(parts) != 6:
                raise ValueError(
                    "Formato inválido. Esperado: 'value from_type and value2 from_type2 _ to_type'"
                )
            ...
            if len(parts) != 4:
                raise ValueError(
                    "Formato inválido. Esperado: 'value from_type _ to_type'"
                )
